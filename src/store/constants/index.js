// Create constants for Workshop
export const CREATE_WORKSHOP_REQUEST = 'src/store/CREATE_WORKSHOP_REQUEST';
export const CREATE_WORKSHOP_SUCCESS = 'src/store/CREATE_WORKSHOP_SUCCESS';
export const CREATE_WORKSHOP_FAILED = 'src/store/CREATE_WORKSHOP_FAILED';
export const GET_WORKSHOP_REQUEST = 'src/store/GET_WORKSHOP_REQUEST';
export const GET_WORKSHOP_SUCCESS = 'src/store/GET_WORKSHOP_SUCCESS';
export const GET_WORKSHOP_FAILED = 'src/store/GET_WORKSHOP_FAILED';
export const DELETE_WORKSHOP_REQUEST = 'src/store/DELETE_WORKSHOP_REQUEST';
export const DELETE_WORKSHOP_SUCCESS = 'src/store/DELETE_WORKSHOP_SUCCESS';
export const DELETE_WORKSHOP_FAILED = 'src/store/DELETE_WORKSHOP_FAILED';
export const GET_WORKSHOP_BY_ID_REQUEST = 'src/store/GET_WORKSHOP_BY_ID_REQUEST';
export const GET_WORKSHOP_BY_ID_SUCCESS = 'src/store/GET_WORKSHOP_BY_ID_SUCCESS';
export const GET_WORKSHOP_BY_ID_FAILED = 'src/store/GET_WORKSHOP_BY_ID_FAILED';
export const UPDATE_WORKSHOP_REQUEST = 'src/store/UPDATE_WORKSHOP_REQUEST';
export const UPDATE_WORKSHOP_SUCCESS = 'src/store/UPDATE_WORKSHOP_SUCCESS';
export const UPDATE_WORKSHOP_FAILED = 'src/store/UPDATE_WORKSHOP_FAILED';

// Create constants for Genre
export const CREATE_GENRE_REQUEST = 'src/store/CREATE_GENRE_REQUEST';
export const CREATE_GENRE_SUCCESS = 'src/store/CREATE_GENRE_SUCCESS';
export const CREATE_GENRE_FAILED = 'src/store/CREATE_GENRE_FAILED';
export const GET_GENERE_REQUEST = 'src/store/GET_GENERE_REQUEST';
export const GET_GENERE_SUCCESS = 'src/store/GET_GENERE_SUCCESS';
export const GET_GENERE_FAILED = 'src/store/GET_GENERE_FAILED';
export const DELETE_GENERE_REQUEST = 'src/store/DELETE_GENERE_REQUEST';
export const DELETE_GENERE_SUCCESS = 'src/store/DELETE_GENERE_SUCCESS';
export const DELETE_GENERE_FAILED = 'src/store/DELETE_GENERE_FAILED';

// Create constants for Users
export const CREATE_USER_REQUEST = 'src/store/GET_WORKSHOP_FAILED';
export const CREATE_USER_SUCCESS = 'src/store/GET_WORKSHOP_FAILED';
export const CREATE_USER_FAILED = 'src/store/GET_WORKSHOP_FAILED';
